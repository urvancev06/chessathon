...
def _init_worker() -> None:
    ...
    # `USE_NETWORK` exists only on a tree whose evaluation carries the network (the
    # `ct-nnue-screen` branch). On `main` the evaluation is the hand-written one and the
    # reference is already fixed, so there is nothing to switch off.
