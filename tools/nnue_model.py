...
**This network does not ship and nothing on `main` calls this module.** Screened against v1.2 over
300 games at the real time control it lost 67 Elo (interval -106 to -29), so the compiled port that
went with it -- a `fasteval.py` carrying `nnue_evaluate` and an `USE_NETWORK` switch in
`evaluation.py` -- was never merged. The working tree where all of it runs together, including the
integer-for-integer parity test, is the `ct-nnue-screen` branch, where this file sits in
`mikhail_letal/` beside `evaluation.py`. `docs/NNUE-NOTES.md` is the post-mortem and it is the more
useful artefact. This file is kept on `main` -- under `tools/`, because `make zip` packages every
`.py` in `mikhail_letal/` and nothing that does not play should be in the submission -- because the
diagnosis in those notes is about this architecture and cannot be read without it.

Read the rest as it was written, on a tree where the compiled half existed.
