...
browser. Nothing is loaded from the network (no fonts, no libraries, no CDN), so it works offline
at a judging table.
...
- **Play**: you against any seat (the working tree, every `versions/*/`, the four baselines, and
  ...
  opening in `data/openings.txt` by index, or a custom FEN validated as you type; time-control
  presets or base/increment fields. The engine runs as a real process through
  ...
  pointer, drops snap), a promotion strip over the target square, takeback, resign, flip, PGN
  download, copy FEN, clocks that tick in the browser (your clock reaching zero ends the game
  by flag), and a thinking strip with depth/seldepth, nodes, nodes/s, time, the soft/hard budget
  bar and sparklines of time and nodes per move from the engine's log line.
- **New game**: the setup form (engine build, colour incl. Random, time control, starting
  position); `#/new?opening=<index>` preselects a curated opening.
- **Spectate**: engine against engine, played by the server in a background thread with the
  ...
  browser polls every 500 ms; Stop pauses the loop between moves (both agents stay alive and the
  clocks stand still), Resume continues, Restart replays the same setup.
...
- **Overview** (identity, real version/commit/build date, the competition contract, how the
  engine works, the latest results table from `docs/RESULTS.md`), **Docs** (`docs/*.md` rendered
  client-side), **Weights** (`weights/pst.json`: piece values, provenance, twelve piece-square
  heatmaps), **Openings** (all 219 rows of `data/openings.txt` with mini boards, a filter and a
  Play link).
...
The front end implements the design handoff in `design_handoff_letal/` (README, `tokens.css`,
the `.dc.html` prototypes): system sans, one accent, hairlines, no cards, the 120 ms settle and
nothing else animating, light and dark themes (the toggle in the nav; "system" follows the OS
until you choose), responsive down to 360 px. `app/css/tokens.css` is a verbatim copy of the
handoff's tokens.

The board has two styles, switched from the "Board · lichess / studio" control in the nav bar
and remembered in `localStorage` (`letal.board`, default `lichess`):

- **lichess** (default): the "brown" theme (`#f0d9b5` / `#b58863`, unchanged in dark mode),
  coordinates inside the squares, lichess's colours for the last move, the selected piece, legal
  destinations, captures and check, and the **cburnett** pieces by Colin M.L. Burnett,
  CC BY-SA 3.0, used unmodified from `app/pieces/cburnett/` (see `LICENSE.txt` there).
- **studio**: the handoff's board — grey squares from the tokens, Unicode glyph pieces at 9cqw,
  the accent last-move overlay, the legal dot at 22 % of the square.

Everything outside the board follows the handoff in both styles; the analysis arrow keeps
chessground's geometry in both.
...
app/index.html              the shell: nav, <main>, the script tags in order
app/css/                    tokens.css (verbatim from the handoff), base.css, board.css, screens.css
app/js/                     util.js (LT namespace, formatting, DOM), api.js, theme.js, board.js,
                            movelist.js, router.js, main.js — vanilla, no build step
app/js/screens/             play, newgame, spectate, overview, docs, weights, openings, analysis
app/favicon.svg             the mark; app/pieces/cburnett/ the piece set (SVG) and its licence
design_handoff_letal/       the design references (open the .dc.html files in a browser)
...
for f in app/js/*.js app/js/screens/*.js; do node --check "$f"; done
```

The server serves `app/` as the static root: `/` is `app/index.html`, any other path is a file
under `app/` (`/css/...`, `/js/...`, `/pieces/...`, `/favicon.svg`); path traversal answers 404.
...
```
GET  /api/info                    engine identity, git state (commit, short, branch, describe, dirty,
                                  built = the HEAD commit's date), RESULTS.md tail, contract, engines,
...
POST /api/games/<id>/takeback | /stop
POST /api/games/<id>/resign     {reason?: "resignation" | "flag"}  flag = the browser saw your clock hit zero
POST /api/games/<id>/pause | /resume   spectate games only; the loop waits between moves, clocks stand still
DELETE /api/games/<id>
...
- Keyboard: on Play, Tab reaches the board, arrows move between squares and Enter selects or
  moves, Escape clears the selection; on Analysis, left/right, Home and End step through the game.
- Theme and board style live in `localStorage` (`letal.theme`, `letal.board`); the last game of
  each kind lives in `sessionStorage`, so `#/play` and `#/spectate` reopen it after a reload.
